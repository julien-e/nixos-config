...

{
  ...
  wayland.windowManager.mango = {
    ...
    settings = ''
      ...
      #bind=SUPER,P,spawn_shell,filename="screenshot-$(date +%Y%m%d-%H%M%S).png" && grim ${config.home.homeDirectory}/Pictures/$filename && notify-send "Screenshot" "Saved: $filename"
      # Screenshot area to file
      bind=SUPER+CTRL,P,spawn_shell,filename="screenshot-$(date +%Y%m%d-%H%M%S).png" && grim -g "$(slurp)" ${config.home.homeDirectory}/Pictures/$filename && notify-send "Screenshot" "Area saved: $filename"
      # Screenshot area to clipboard
      bind=SUPER+SHIFT,P,spawn_shell,grim -g "$(slurp)" - | wl-copy && notify-send "Screenshot" "Area copied to clipboard"
